...
for (k in 1:N){
  ...
  print(sum(queue_time_store!=-1)/length(queue_time_store)*100)
  print(sum(wait_time_store!=-1)/length(queue_time_store)*100)
  print(sum(base_store!=-1)/length(queue_time_store)*100)
...
# Listan jokaiseen elementtiin k talletaaan seuraavat suureet:
# -wait_times: odotusajat eli kuinka kauan potilas joutuu odottamaan
# -queue_times: jonotusaika dronelle eli kuinka kauan drone joutuisi odottamaan edellisen dronen paluuta siinä nopeimmassa vaihtoehdossa
# -locations: tapahtuman karttakoordinaatti
# -bases: ko. tapahtumaa vastaava base, josta saadaan nopein kuljetus

# Kaikissa paitsi locations-muuttujassa arvona -1 mikäli tapahtumaa ei saada hoidettua kantavuuden rajoissa
