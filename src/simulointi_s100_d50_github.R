...
# Read the trauma intensities from file and initialize
##############################################################
predictions=read.csv(file = "../data/trauma_prob_1x1.csv",header = TRUE,sep=";",dec=",")

# Transform the number of events per year to number of events per minute
predictions$scaled=predictions$trauma_saatto_enn/(365*24*60)       

# For numerical reasons, the change the zero into a very small positive value 
predictions$scaled[predictions$scaled==0]=1.119e-09/(365*24*60) 
intensities <- predictions$scaled

# Read the grid distances from potential BDC's
load('../data/et_mat.RData')
...
N_base=ncol(et_mat)           # Number of BDC's
...
for (k in 1:N){
  ...
  for (i in 1:N_ev) {
    ...
    travel_times=et_mat[location_index,]/speed*60                    # Travel (flight) times in minutes
